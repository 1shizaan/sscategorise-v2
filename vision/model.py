...
_model_cache = None  # Module-level cache so we don't reload every call 
...
def get_image_features(image_path: str, model=None) -> np.ndarray: 
    """ 
    Returns 1280-dim feature vector from EfficientNet-B0 feature extractor. 
    This is the CONTRACT function — Fusion Engineer calls this. 
    """ 
    if model is None: 
        model = load_model() 
    try: 
        ...
        return np.zeros(1280)  # Return zeros so pipeline doesn't crash 
    tensor = transform(img).unsqueeze(0) 
    with torch.no_grad(): 
        feats = model.features(tensor) 
        feats = torch.nn.functional.adaptive_avg_pool2d(feats, 1) 
        feats = feats.squeeze().numpy()   # shape: (1280,) 
    return feats 
